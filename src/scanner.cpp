					  ...
					  bool offset_tuning, int gain, int ppm_error, window_types win_type)
{
	...
	status = frequency_range(crop, upper_freq, lower_frep, bin_len);
	...
	if (!m_rtl_device.device_connected()){
		status = m_rtl_device.open_device(rtl_dev_index);
		if(status != RTL_OK)
	...
	if (gain == -1){
		...
		status = m_rtl_device.set_gain(gain * 10);
		if(status != RTL_OK)
			return SCANNER_DEVICE_ERROR;
	}

	if(ppm_error != 0){
		status = m_rtl_device.set_ppm(ppm_error);
		if(status != RTL_OK)
			return SCANNER_DEVICE_ERROR;
	}
